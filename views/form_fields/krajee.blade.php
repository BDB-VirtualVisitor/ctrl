...
<script type="text/javascript">
  ...
  @if (!empty($field['value']))
    ...
    @elseif ($field['type'] == 'file')
      btnCust = '<a href="{{ route('ctrl::file',[$field['value']]) }}" class="btn btn-default" id="fileinput-custom-download_{{ $field['id'] }}"><i class="fa fa-download"></i> Download</a>';
    @endif
  @endif
  ...
  $('#{{ $field['id'] }}_krajee').fileinput({
    ...
    if ($('#{{ $field['id'] }}').attr('name').indexOf('[]') > -1) { // If this is a multiple field
      ...
      new_field.val(response.path);
      $('#{{ $field['id'] }}').after(new_field);
    }
    else {
      $('#{{ $field['id'] }}').val(response.path);
    }
