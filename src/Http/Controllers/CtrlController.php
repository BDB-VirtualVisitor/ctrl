<?php namespace Phnxdgtl\Ctrl\Http\Controllers;
/**
 *
...
class CtrlController extends Controller
{
	...
	public function __construct(CtrlModules $ctrlModules) {
		...
		/* We can now run modules lke this:
		...
		*/
		...
		$this->disk = config('ctrl.disk','public');
	...
	protected function build_menu() {
		...
		if ($this->module->enabled('custom_menu_links')) {
			if ($custom_menu_links = $this->module->run('custom_menu_links')) {
				$menu_links = array_merge($menu_links, $custom_menu_links);
			}
		}
	...
	protected function _check_login($user = null) {
		...
		if ($this->module->enabled('_check_login')) {
			return $this->module->run('_check_login', [
				$user
			]);
		} else {
	...
	public function list_objects(Request $request, $ctrl_class_id, $filter_string = NULL)
	{
		...
		if ($filter_description) {
			$page_description = 'Showing all '. $ctrl_class->get_plural() . $filter_description .'.';
		}
        ...
        foreach ($headers as $header) {
			...
			// NOTE: we'd ideally check for a custom field type here, but we can't, because
			// (a) we don't have $object, and (b) we're setting up table headers, and a column
			// could contain a mixture of field types once we start to modify them.
        	...
        	if ($header->relationship_type) {
				...
				else if ($header->field_type == 'dropdown' && $enums = $this->getEnumValuesFromProperty($header)) {
        			$th_columns[] = '<th data-search-dropdown="'.($make_searchable ? 'true' : 'false').'" data-orderable="false">'.$header->label.'</th>';
        		}				
        		else {
        			$th_columns[] = '<th data-search-text="'.($make_searchable ? 'true' : 'false').'" data-orderable="'.($make_orderable ? 'true' : 'false').'">'.$header->label.'</th>';
		...
		if (!empty($custom_columns)) {
			foreach ($custom_columns as $custom_column=>$details) {
				...
				$th_column = sprintf('<th %s>%s</th>',($details['searchable'] ? 'data-search-text="true"' : ''),$details['table_heading']);
				if (!empty($details['position'])) {
					array_splice( $th_columns, $details['position'], 0, $th_column);
					array_splice( $js_columns, $details['position'], 0, [$cc]);
				} else {
		...
		*/
		...
		if ($this->module->enabled('last_import_date')) {
			$page_description .= ' '.$this->module->run('last_import_date',[
				$ctrl_class
			]);
		}
	...
	public function export_objects(Request $request, $ctrl_class_id, $filter_string = NULL) {
		...
		/**
		 * This is lifted extensively from get_data, we have quite a bit of duplicate code here
		 */
			...
			foreach ($filter_array as $filter) {
				...
				if (!empty($filter_ctrl_property->relationship_type)) {
					...
					if ($filter_ctrl_property->relationship_type == 'belongsTo') {
						...
						$objects->where($filter_ctrl_property->foreign_key,$related_object->id);
					}
						...
						$objects->join($filter_ctrl_property->pivot_table, 'id', '=', $filter_ctrl_property->pivot_table.'.'.$filter_ctrl_property->local_key)
					...
					// Filter by property...
					$objects->where($filter_ctrl_property->name,$filter['value']);
				}
			}
		}
		...
		if ($this->module->enabled('filter_export_objects')) {
			...
			/**
			 * Make sure filter_export_objects always returns the $objects object,
			 * even if we don't actually filter this class
			 */
			if (is_null($objects)) {
				trigger_error("\$objects set to null following the filter_export_objects callback");
			}
		}
		...
		if (!empty($custom_columns)) {
			foreach ($custom_columns as $custom_column=>$details) {
				...
				if (!empty($details['raw_sql'])) {
					$add_select = \DB::raw(sprintf('IFNULL((%s), \'None\') AS `%s`',$details['raw_sql'],$custom_column));
					$objects->addSelect($add_select);
				} else {
					$add_select = \DB::raw(sprintf('(\'empty\') AS `%s`',$custom_column));
					$objects->addSelect($add_select);
				}				
				$headers[] = $details['table_heading'];
			}
		}
	...
	protected function import_objects_process_data(Request $request, $ctrl_class_id, $filter_string = NULL) {
		...
		$slugged_headers = array_map('Illuminate\Support\Str::slug',$required_headers,
	...
	public function populate_datatables_dropdowns(Request $request, $ctrl_class_id, $filter_string = NULL) {
		...
		if (strpos($data_src, '.')) { // Suggests a related property, like brand.title
			...
			// $data_src here is the property name, so check to see if it's an ENUM, and return results accordingly
			$enum_property = CtrlProperty::where('ctrl_class_id', $ctrl_class->id)->where('name', $data_src)->first();
			
			if (!is_null($enum_property) && $enums = $this->getEnumValuesFromProperty($enum_property)) {
				$options = $enums;
			} else {
	 ...
	 */
	    		...
	    		if ($src = $object->$imageColumn) {
					...
					if (strpos($src,'http') === 0) {
						...
						*/
						...
						$url  = Storage::disk($this->disk)->url($src);

						$cache_key = md5($object->id.$imageColumn.$src);
						$file_missing = Cache::remember($cache_key, 60 * 60 * 24 * 365, function () use ($src) {
							return !Storage::disk($this->disk)->exists($src);
						});
					...
					$basename   = Str::limit($path_parts['basename'],44);
	    		...
	    		if ($video_src = $object->$videoColumn) {
					...
					if (!empty($object->$thumbnail)) {
						...
						if (strpos($src,'http') === 0) {
							...
							$cache_key = md5($object->id.$thumbnail.$src);
							$file_missing = Cache::remember($cache_key, 60 * 60 * 24 * 365, function () use ($src) {
								return !Storage::disk($this->disk)->exists($src);
							});

							if ($file_missing) {
								return 'Video missing <!--'.$path.'-->';
							}
						}

						return sprintf('<div class="media"><div class="media-left"><a href="#" class="videoModal" data-toggle="modal" data-target="#videoModal" data-video="%1$s"><img class="media-object" src="%2$s" height="30"></a></div></div>',Storage::disk($this->disk)->url($video_src), $url);
					} else {
						return sprintf('<a href="#" class="videoModal" data-toggle="modal" data-target="#videoModal" data-video="%1$s">%2$s</a>',Storage::disk($this->disk)->url($video_src), $video_src);
					}
				}
        	});
        }
        	...
        	$datatable->editColumn($fileColumn, function($object) use ($fileColumn) {  // If we have a "file" column, assume it's a clickable link. DEFINITELY need to query ctrlproperty->type here,see 'src' above:
	    		if ($src = $object->$fileColumn) {
	    			...
	    			if (strpos($src,'http') === 0) {
						...
						$cache_key = md5($object->id.$fileColumn.$src);
						$file_missing = Cache::remember($cache_key, 60 * 60 * 24 * 365, function () use ($src) {
							return !Storage::disk($this->disk)->exists($src);
						});
		...
		foreach ($textColumns as $textColumn) {
        	$datatable->editColumn($textColumn, function($object) use ($textColumn) {
				return Str::words(html_entity_decode(strip_tags($object->$textColumn)), 10, '...');	    		
        	});
		}
	...
	public function delete_object($ctrl_class_id, $object_id) {
        ...
        if ($this->module->enabled('post_delete')) {
			$this->module->run('post_delete', [
				$object
			]);
		}
	...
	protected function convert_txt_to_csv($csv_file) {
		...
		$url = str_replace(" ", "%20",Storage::disk($this->disk)->url($csv_file));
		$fh = fopen($url, 'r');
		 ...
		 */
		Storage::disk($this->disk)->put($csv_file, $csv_rows);
	...
	protected function detect_utf_encoding($filename) {
		...
		$text = Storage::disk($this->disk)->get($filename);
	...
	protected function get_object_title($object) {
		...
		foreach ($title_properties as $title_property) {
			...
			if ($title_property->relationship_type == 'belongsTo') {
				...
				if (in_array($title_property->field_type, ['text', 'froala'])) {
					$title_strings[] = Str::words(html_entity_decode(strip_tags($object->$property)), 10, '...');
				} else {
	...
	/**
	 * Given a property, do we have an associated list of ENUM values?
	 * This only returns an array of values for ENUM fields, obviously
	 * @param	CtrlProperty $ctrl_property
	 * @return boolean|array 
	 */
	protected function getEnumValuesFromProperty($ctrl_property) {
	...
	public function edit_object($ctrl_class_id, $object_id = NULL, $filter_string = NULL)
	{
		...
		foreach ($ctrl_properties as $ctrl_property) {
			...
			/**
			 * Load the field type, in case we've modified it via a Module
			 */
			$field_type = $this->getFieldType($ctrl_class, $ctrl_property, $object);
			...
			*/
			...
			elseif (in_array($field_type, ['date','datetime','time'])) {
			...
			elseif (empty($field_type)) {
				...
				$ctrl_property->template = $field_type;
			}

			if (!view()->exists('ctrl::form_fields.'.$ctrl_property->template)) {
				trigger_error("Cannot load view for field type ".$field_type);
			}
			...
			if ($ctrl_property->related_to_id) {
				$related_ctrl_class = \Phnxdgtl\Ctrl\Models\CtrlClass::find($ctrl_property->related_to_id);
				$related_class 		= $related_ctrl_class->get_class();
				...
				*/
				...
				$enums = $this->getEnumValuesFromProperty($ctrl_property);
				if ($enums) {
					foreach ($enums as $enum) {						
			...
			$tip = $this->getTip($ctrl_class, $ctrl_property, $object);

			$label = $this->getLabel($ctrl_class, $ctrl_property, $object);
			...
			$tabbed_form_fields[$tab_name]['form_fields']['form_id_'.$ctrl_property->name] = [
				...
				'type'                    => $field_type, // This is used to modify some templates; date.blade.php can handle date, datetime or time types for example
		...
		if ($default_values) { // Set HIDDEN fields here; we can default known fields in the main loop above
			...
			foreach ($default_values as $default_value) {
				$default_property = $ctrl_class->ctrl_properties()->
									// where('fieldset','')->
									where(function($q) { 
										$q->whereNull('fieldset')->orWhere('fieldset','');
									})->
	...
	public function save_object(Request $request, $ctrl_class_id, $object_id = NULL, $filter_string = NULL)
	{
		...
		*/
			...
			*/
			if ($field_type == 'email') {
				if (in_array('required', $flags)) {
					...
					// Allow empty emails
					$validation[$field_name][] = 'nullable|email';
				}
			...
			else if (in_array($field_type, ['time'])) {
				if (in_array('required', $flags)) {
		...
		foreach ($ctrl_properties as $ctrl_property) {
			...
			if (in_array($field_type, ['date','datetime','time']) && !empty($_POST[$ctrl_property->name])) {
				switch ($field_type) {
					case 'date':
						$date_format = 'Y-m-d';
						break;
		...
		/**
		 * This is a bit wasteful now, we used to filter by field_type but can't,
		 * now that we can modify it via a Module. Not the end of the world though.
		 */
		...
		foreach ($check_boolean_properties as $check_boolean_property) {
			$field_type = $this->getFieldType($ctrl_class, $check_boolean_property, $object);
		...
		foreach ($related_ctrl_properties as $related_ctrl_property) {
	        ...
	        if ($request->input($related_field_name)) {
	        	...
	        	$related_ctrl_class = \Phnxdgtl\Ctrl\Models\CtrlClass::find($related_ctrl_property->related_to_id);
		...
		if ($request->input('submit_type') == 'continue') {
			$redirect = route('ctrl::edit_object',[$ctrl_class->id, $object->id, $filter_string]);
		}
		else if ($ctrl_class->can('list')) {
	...
	public function froala_upload(Request $request)
	{
		...
		// Don't really want 'bin' here, but Laravel assesses some CSV files (Windows ones?!) as 'bin'
		$rules = [
            'file' => ['mimes:gif,jpg,jpeg,bmp,png,txt,csv,xls,doc,pdf,bin,mp4']
        ];

		if (config('ctrl.clamav')) {
			$rules['file'][] = 'clamav';
		}
        ...
        if ($validator->fails()) {
			// Actual $error is here: $validator->errors()->first();
            $response->error = 'That file type cannot be uploaded <!-- '.$validator->errors()->first().' -->';
        } else if ($request->file('file')->isValid()) {
			$path           = $this->upload($request, 'file');
			$response->link = Storage::disk($this->disk)->url($path);
		}
		else {
	...
	public function krajee_upload(Request $request)
	{
		...
		// Don't really want 'bin' here, but Laravel assesses some CSV files (Windows ones?!) as 'bin'
		$rules = [
            $field_name => ['mimes:gif,jpg,jpeg,bmp,png,txt,csv,xls,doc,pdf,bin,mp4']
        ];

		if (config('ctrl.clamav')) {
			$rules[$field_name][] = 'clamav';
		}

		$validator = Validator::make($request->all(), $rules);

        if ($validator->fails()) {
			// $error is here: $validator->errors()->first();            
			$response->error = 'That file type cannot be uploaded <!-- '.$validator->errors()->first().' -->';
        } else if ($request->hasFile($field_name)) {
	...
	protected function upload($request, $fieldName) {
		...
		if ($request->type == 'image') {
			// $path = $request->file($fieldName)->store('images',$this->disk);
			// This way is more compatible with S3, as we can set the "public" flag
			// Note that for some S3 buckets (eg Multi-Tenant), this won't work
			// and we'll get a permissions error back from S3. So, check for "visibility"
			if (config('filesystems.disks.'.$this->disk.'.visibility', false) == 'public') {
				$options = 'public';
			} else {
				$options = [];	
			}	
			$path = Storage::disk($this->disk)->putFile('images', $request->file($fieldName), $options);		
		}
			...
			$path = Storage::disk($this->disk)->putFileAs('files', $request->file($fieldName), $fileName, $options);			
		}
	...
	{		
		if ($this->module->enabled('login')) {
			return $this->module->run('login', [
				$request
			]);
		} else {
			$user          = Auth::user();
	...
	public function post_login(Request $request)
	{
		...
		if ($this->module->enabled('post_login')) {
			return $this->module->run('post_login', [
				$request
			]);
		} else {			
			...
			if (config('ctrl.forget_login', false)) {
				$remember = false;
			} else {
			...
			/**
			 * Check for account lockout; this is quite crude, but Argos need it
			 */
			$user = DB::table('users')->where('email', $email)->first();
			if (!is_null($user) && property_exists($user, 'locked') && $user->locked) {
				$redirect = route('ctrl::login');
				$error  = 'Your account is locked; please contact support';
				$status = 401;
			}
			else if (Auth::attempt(['email' => $email, 'password' => $password], !empty($remember))) {
				...
				if ($login_attempts >= 5 && Schema::hasColumn('users', 'locked')) {
					DB::table('users')->where('email', $email)->update([
						'locked'=>true
					]);
				}
				$status = 401;
				$request->session()->put('login_attempts', $login_attempts + 1);
				$error  = 'Details not recognised';					
			}
	...
	public function test_models()
	{
		...
		// Ones
		$one = \Phnxdgtl\Ctrl\Models\One::find(1);
		foreach ($one->test as $t) {
			dump($t->title);
		}

		// Manies
		$many = \Phnxdgtl\Ctrl\Models\Many::find(1);
	...
	public function image($mode,$file)
	{
		...
		if ($mode == 'view') {
			if (!Storage::disk($this->disk)->exists($file)) {				
				...
				// Handle spaces in filenames, from https://stackoverflow.com/a/27124836/1463965
				$url = preg_replace_callback('/[^\x21-\x7f]/', function($match) {
					return urlencode($match[0]);
				}, $url);

				$image = Image::make($url)->resize(600, 600, function ($constraint) {
	...
	/**
	 * As per @image, allow us to download a file. Only one $mode though, 'view' not available
	 * @param mixed $file 
	 * @return mixed 
	...
	/**
	 * Allow us to adjust the field type of a ctrl_property on the fly
	 * This would ideally be a method of CtrlProperty, but those models
	 * don't currently have access to the CtrlModules file.
	 * @param object $ctrl_class 
	 * @param object $ctrl_property 
	 * @param object $object The object we're editing, or listing
	 * @return mixed 
	 */
	protected function getFieldType($ctrl_class, $ctrl_property, $object) {
		if ($this->module->enabled('custom_field_type')) {
			$field_type = $this->module->run('custom_field_type', [
				$ctrl_class,
				$ctrl_property,
				$object
			]);
		...
		if (empty($field_type)) {
			$field_type = $ctrl_property->field_type;
		}
		return $field_type;			
	}

	/**
	 * Allow us to adjust the tip of a ctrl_property on the fly
	 * This would ideally be a method of CtrlProperty, but those models
	 * don't currently have access to the CtrlModules file.
	 * @param object $ctrl_class 
	 * @param object $ctrl_property 
	 ...
	 * @return mixed 
	 */
	protected function getTip($ctrl_class, $ctrl_property, $object) {
		if ($this->module->enabled('custom_tip')) {
			$tip = $this->module->run('custom_tip', [
				$ctrl_class,
				$ctrl_property,
				$object
			]);
	...
	/**
	 * Allow us to adjust the label of a ctrl_property on the fly	 
	 ...
	 * @return mixed 
	 */
	protected function getLabel($ctrl_class, $ctrl_property, $object) {
		if ($this->module->enabled('custom_label')) {
			$label = $this->module->run('custom_label', [
				$ctrl_class,
				$ctrl_property,
				$object
			]);
